...
class VisitState {
  constructor(types, sourceFilePath, inputSourceMap, ignoreClassMethods = [], cvIncreaseCb) {
  ...
  increase(type, id, index) {
    ...
    if (this.cvIncreaseCb) {
      ...
      return T.callExpression(T.memberExpression(T.identifier(this.varName), T.identifier('inc')), args);
    } else {
...
function programVisitor(types, sourceFilePath = 'unknown.js', opts = defaultProgramVisitorOpts) {
  ...
  const visitState = new VisitState(types, sourceFilePath, opts.inputSourceMap, opts.ignoreClassMethods, opts.cvIncreaseCb);
  let cvTemplate;
  let cvNode;

  if (opts.cvIncreaseCb) {
    cvTemplate = (0, _template.default)(opts.cvIncreaseCb);
    cvNode = cvTemplate({
  ...
  return {
    ...
    exit(path) {
      ...
      if (opts.cvIncreaseCb) {
        coverageNode.properties.push(T.ObjectMethod('method', T.identifier('inc'), [T.identifier('type'), T.identifier('id'), T.identifier('index')], T.BlockStatement(cvNode)));
      }
